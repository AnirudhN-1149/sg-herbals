...
export default function InventoryPage() {
  ...
  const exportInventoryPDF = () => {
    const doc = new jsPDF()
    doc.text('SG Herbals - Raw Materials Inventory', 14, 15)
    ...
    doc.autoTable({
      head: [['Material Name', 'Unit', 'Qty', 'Cost/Unit', 'Subtotal']],
      body: tableData,
      startY: 20,
      theme: 'grid',
      headStyles: { fillColor: [43, 83, 65] }
    })
    doc.save('sg_herbals_inventory.pdf')
  }

  const exportSalesPDF = () => {
    const doc = new jsPDF()
    doc.text('SG Herbals - Recent Sales', 14, 15)
              ...
              <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/20 airy-shadow-low overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-outline-variant/20 bg-surface-container-low/50">
                  ...
                  <div className="flex items-center gap-3">
                    <button onClick={exportInventoryPDF} className="flex items-center gap-2 px-4 py-2 bg-surface-container rounded-lg text-label-sm font-label-sm text-on-surface-variant hover:bg-surface-container-high transition-colors">
                      <span className="material-symbols-outlined text-[18px]">download</span>
                      Export PDF
                    </button>
                    <button onClick={addMaterial} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-on-primary text-label-sm font-label-sm hover:opacity-90 transition-opacity airy-shadow-low">
            ...
            <div>
              ...
              <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/20 airy-shadow-low overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-outline-variant/20 bg-surface-container-low/50">
                  ...
                  <button onClick={exportSalesPDF} className="flex items-center gap-2 px-4 py-2 bg-surface-container rounded-lg text-label-sm font-label-sm text-on-surface-variant hover:bg-surface-container-high transition-colors">
                    <span className="material-symbols-outlined text-[18px]">download</span>
                    Export PDF
                  </button>
                </div>
                  ...
                  <thead>
                    <tr className="border-b border-outline-variant/20">
                      {['Order ID', 'Customer', 'Date', 'Total Amount', 'Status'].map(h => (
                        <th key={h} className="px-6 py-4 text-label-md font-label-md text-on-surface-variant">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-outline-variant/10">
                    {salesData.recentOrders && salesData.recentOrders.filter(o => o.status === 'completed').map((o) => (
                      <tr key={o._id} className="hover:bg-surface-container-low/30 transition-colors">
